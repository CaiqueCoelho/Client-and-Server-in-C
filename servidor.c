...
int main(int argc, char **argv){
	...
	int port = 6774; /* can be used without root privileges */
	...
	while(1){ /* infinite server loop */
		...
		char *bufferFile = 0;
		bufferFile = malloc (sz);
  		if (bufferFile)
  		{
    		fread (bufferFile, 1, sz, file);
  		}
		...
		send(sockfd, contentType, strlen(contentType), 0);
		send(sockfd, contentLength, strlen(contentLength), 0);

		send(sockfd, bufferFile, strlen(bufferFile), 0);
		send(sockfd, "\r\n", 2, 0);
